...
\begin{itemize}
    ...
    \item $b_t$ is the number of cache faults of the LRU eviction policy if we had a dedicated cache of some promised size.
    \item $p_t$ is the priority of the tenant, which can be defined in terms of the cost of a cache fault for the tenant, or the importance of the tenant. If there is no need to prioritize tenants, we can set $p_t = 1$ for all tenants, in case it is needed, priorities can be defined based on how importanat is for each tenant to achieve the promised hit ratio.
...
\begin{itemize}
    \item \textbf{Chapter 2: Algorithm Overview} - Will describe the base algorithm that we used, and discuss tenant selection and cache eviction policies.
    \item \textbf{Chapter 3: Experiments and Results} - Will describe the datasets we used, including the data collection and processing parts, the performed experiments, and the results.
    \item \textbf{Chapter 4: Conclusions and Recommendations} - Will summarize the results and propose recommendations for future work.
\end{itemize}