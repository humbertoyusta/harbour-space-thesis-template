        ...
        \begin{algorithmic}
            ...
            \ELSE
                ...
                \IF {There is available space in cache}
                    \STATE $\text{available\_location} = \text{GetAvailableBufferLocation}()$ \COMMENT {Get the first available buffer location}
...
$$
\left( \frac{s_t - b_t}{b_t} \right) ^2 \cdot p_t \, \cdot \, \text{sgn}( s_t - b_t )
$$

Where:

\begin{itemize}
    \item $s_t$ --- is the number of faults of the tenant in the current solution that is being evaluated.
    \item $b_t$ --- is the number of faults of the tenant in a LRU cache of the promised size (or $x\%$ bigger than the recommendation).
    \item $p_t$ --- is the priority of the tenant.
...
$$
\left( \frac{l_t - h_t}{l_t} \right) ^2 \cdot p_t \, \cdot \, \text{sgn}( l_t - h_t )
$$

Where:

\begin{itemize}
    \item $h_t$ --- is the number of hits of the tenant in the current solution that is being evaluated.
    \item $l_t$ --- is the number of hits of the tenant in a LRU cache of the promised size (or $x\%$ bigger than the recommendation).
    \item $p_t$ --- is the priority of the tenant.
    \item $\text{sgn}(x)$ --- is the sign function, returning $1$ if $x > 0$, $-1$ if $x < 0$ and $0$ if $x = 0$.
\end{itemize}
...
$$
\frac{ \left( \frac{s_t - b_t}{b_t} \right) ^2 \cdot p_t \, \cdot \, \text{sgn}( s_t - b_t ) }{ \left( \frac{c_t}{d_t} \right) ^2 }
$$
...
\begin{itemize}
    ...
    \item $p_t$ --- is the priority of the tenant.
    \item $\text{sgn}(x)$ --- is the sign function, returning $1$ if $x > 0$, $-1$ if $x < 0$ and $0$ if $x = 0$.
    \item $c_t$ --- is the number of pages currently in the cache of the tenant.
    \item $d_t$ --- is the size of the LRU cache of the promised size (or $x\%$ bigger than the recommendation) for the tenant.
\end{itemize}
...
Where $c_t$ is the number of pages currently in the cache of the tenant, and
$d_t$ is the size of the LRU cache of the promised size (or $x\%$ bigger than
the recommendation) for the tenant.
